...
countTransactionsForAddress(const otIp6Address* addr, list_t transactions_list) {
	coap_transaction_t *t = NULL;
	uint8_t counter = 0;
	  for (t = (coap_transaction_t*)list_head(transactions_list); t; t = t->next){
	    //if (uip_ipaddr_cmp(addr, &t->addr)){
		if (memcmp(addr, &t->endpoint.ipaddr, sizeof(otIp6Address) == 0)) {
	     counter++;
	    }
	  }
	  return counter;
...
coap_delete_rtt_by_freshness(void) {
...
coap_new_rtt_estimation(clock_time_t rtt, otIp6Address* addr, uint8_t retransmissions)
{
  ...
  if (e){
	...
	memcpy(&e->addr, addr, sizeof(otIp6Address));
...
coap_update_rtt_estimation(otIp6Address* transactionAddr, clock_time_t rtt, uint8_t retransmissions){
	coap_rtt_estimations_t *t = NULL;
	PRINTF("Measured RTT:%lu\n", (unsigned long) rtt);
		  for (t = (coap_rtt_estimations_t*)list_head(rtt_estimations_list); t; t = t->next){
		    // if (uip_ipaddr_cmp(transactionAddr, &t->addr)){
			if (memcmp(transactionAddr, &t->addr, sizeof(otIp6Address)) == 0) {
...
coap_check_rtt_estimation(otIp6Address* transactionAddr, list_t transactions_list) {
	coap_rtt_estimations_t *t = NULL;
	uint8_t destCount = 0;
	    	...
	    	if(t->rto[OVERALLESTIMATOR] == COAP_INITIAL_RTO){
	    		...
	    		// already directed to the same destination node, increase the multiplier (Blind RTO)
		    	  ...
		    	  for (trans = (coap_transaction_t*)list_head(transactions_list); trans; trans = trans->next){
					...
					if (memcmp(transactionAddr, &trans->endpoint.ipaddr, sizeof(otIp6Address)) == 0) {
